...
| :-------------------- | :--------------------------- | :---------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. 音声認識 (ASR)** | **リアルタイム音声認識**     | `nodejs-whisper` (Local) / **Groq Cloud** | 1. **VAD (無音検知):** `@ricky0123/vad-web` 等で発話区間のみを検出し、Whisper (CPU/API) の無駄な処理を抑制する。<br>2. **処理:** ローカル (`whisper-server.exe`) または クラウド (Groq/OpenAI API) を選択可能。<br>3. **モデル:** ローカルは `tiny`/`base` 推奨、クラウドは `whisper-large-v3-turbo` 等を使用。 |
...
| カテゴリ          | 機能名                   | 詳細                                                                                                                                                                  |
| :---------------- | :----------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A. 設定 UI**    | **API・モデル設定**      | 1. **ASRプロバイダー:** Local / Groq API / OpenAI API から音声認識エンジンを選択。<br>2. **LLM設定:** 翻訳用モデル (`llama-3.3-70b` 等) を選択。<br>3. **翻訳方向:** 自動判定 / 固定設定の切り替え。 |
