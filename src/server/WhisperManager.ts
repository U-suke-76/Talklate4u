...
export class WhisperManager extends EventEmitter {
    ...
    async start(language?: string): Promise<void> {
        ...
        // Custom Args Parsing
        const extraArgs = config.whisper.extraArgs ? config.whisper.extraArgs.split(' ').filter(s => s.trim().length > 0) : [];
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.process.stderr?.on('data', (d) => {
                    ...
                    if (msg.toLowerCase().includes('error') || msg.toLowerCase().includes('fail') || msg.toLowerCase().includes('unknown')) {
