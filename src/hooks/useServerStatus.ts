...
export function useServerStatus(
  ...
  const checkServer = useCallback(async () => {
    ...
    if (res.isDownloading && res.progress) {
      ...
      const isStuckDownloading = statusText.startsWith("Downloading") || statusText.includes("Starting download");
    ...
    if (res.success && !lastServerStatus.current) {
      ...
      if (!cfg?.llm?.apiKey) {
        addSystemLog(
          "WARNING: Groq API Key is missing. Translation will not work."
        );
      }
      ...
      !res.success &&
      !res.isDownloading &&
      lastServerStatus.current !== false
    ) {
      ...
      if (res.configIssues && res.configIssues.length > 0) {
        ...
        addSystemLog(
          "Tip: Ensure Whisper Server is running (Settings -> Save & Restart)."
        );
      }
    }
