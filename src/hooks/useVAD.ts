...
export function useVAD() {
    ...
    const startVAD = useCallback(async (
        ...
        const patchedGetUserMedia = async (constraints: MediaStreamConstraints | undefined) => {
            log(`[VAD Init] getUserMedia called by library. Constraints: ${JSON.stringify(constraints)}`);
                ...
                onSpeechEnd: (audio: Float32Array) => {
                    ...
                    if (volumeThreshold > 0 && audio.length > 0) {
                        ...
                        if (volumePercent < volumeThreshold) {
                            log(`[VAD] Audio ignored: volume ${volumePercent.toFixed(1)}% < threshold ${volumeThreshold}%`);
                            return;
                        }
                        log(`[VAD] Audio accepted: volume ${volumePercent.toFixed(1)}% >= threshold ${volumeThreshold}%`);
                    }
