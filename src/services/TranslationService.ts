...
export class TranslationService {
    ...
    private getLLMClient(): OpenAI | null {
        ...
        if (!this.openaiClient) {
            ...
            console.log(`[TranslationService] LLM Client initialized (Provider: ${provider}, BaseURL: ${clientConfig.baseURL || 'Default OpenAI'})`);
        }
    ...
    public async translate(text: string, detectedLanguage?: string): Promise<{ text?: string | null; error?: string }> {
        ...
        if (isAuto) {
             availableModels = (config.llm.groqModels && config.llm.groqModels.length > 0) 
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                return { text: result };
                 ...
                 console.error(`[LLM Error] (Model: ${isAuto ? availableModels[this.currentModelIndex % availableModels.length] : currentModel})`, err);
                 
                 const errorObj = err as { status?: number; message?: string };
                
                if (errorObj.status === 429) {
                    if (isAuto && attempt < maxRetries - 1) {
                        const nextModel = availableModels[(this.currentModelIndex + 1) % availableModels.length];
